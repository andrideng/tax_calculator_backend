...
// Transactional return a handlers that encloses the nested handlers with a DB transaction.
// If a nested handler returns an error or panic happens, it will rollback the transation.
...
func Transactional(db *dbx.DB) routing.Handler {
	return func(c *routing.Context) error {
		...
		if err != nil || rs.Rollback() {
			// rollback if a handler returns an error or rollback explicitly requested.
			e = tx.Rollback()
		} else {
			e = tx.Commit()
		}
